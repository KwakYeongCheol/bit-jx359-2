...
<head>
	...
	<link href="${pageContext.request.contextPath }/resources/css/core-0.0.1.css" rel="stylesheet" type="text/css" media="screen" />
...
<body>
	<header>
		<div class="menu">
			...
			<div class="userblog_admin">
				<c:if test="${loginUserProvider.loggedIn }">
					<c:if test="${loginUserProvider.loginUser.loginId == blog.owner }">
						<a href="${pageContext.request.contextPath }/${blog.id}/admin"
							class="admin">관리자</a>
					</c:if>
				</c:if>
			</div>
				...
				<nav class="nav">
					...
					<ul class="nav-menu">
						<c:forEach items="${loginUserProvider.blogList }" var="blog">
							<li><a
								href="${pageContext.request.contextPath }/${blog.id }">${blog.title
									}</a></li>
						</c:forEach>
						<hr />
						<li><a
							href="${pageContext.request.contextPath }/settings/blog">내
								블로그 관리</a></li>
						<hr />
						<c:forEach items="${loginUserProvider.loginUser.favoriteList }"
							var="favorite">
							<li><a
								href="${pageContext.request.contextPath }/${favorite.blog.id }">${favorite.blog.title
									}</a></li>
						</c:forEach>
						<hr />
						<li><a
							href="${pageContext.request.contextPath }/settings/favorite">이웃
								블로그 설정</a></li>
					</ul>
				</nav>
			</c:if>
			<div class="clear"></div>
			<div class="menu_bar">
				<div class="visitorCount">today : ${blog.todayCount } / total
					: ${blog.totalCount }</div>
				<nav class="nav2">
